...
def score_scenarios(args: List[Dict[str, Union[List[str], DictConfig]]]) -> List[CacheResult]:
    """
    Performs the caching of scenario DB files in parallel.
    :param args: A list of dicts containing the following items:
        "scenario": the scenario as built by scenario_builder
    ...
    def cache_scenarios_internal(args: List[Dict[str, Union[Path, DictConfig]]]) -> List[CacheResult]:
        ...
        def cache_single_scenario(
            scene_dict: List[Dict[str, Any]], processor: MetricCacheProcessor
        ) -> Optional[CacheMetadataEntry]:
            scene = Scene.from_scene_dict_list(
                scene_dict,
                None,
        ...
        tokens = [t for a in args for t in a["tokens"]]
        cfg: DictConfig = args[0]["cfg"]
        if cfg.agent['_target_'] == 'navsim.agents.human_agent.HumanAgent':
            file_name = 'token_scores_human.json'
            print('saving human scores')
        else:
            file_name = 'token_scores_cv.json'
            print('saving cv scores')
        ...
        for idx, (token, scene_dict) in enumerate(scene_loader.scene_frames_dicts.items()):
            ...
            pdm_result = pdm_score(
                metric_cache=file_cache_metadata,
                model_trajectory=trajectory,
                future_sampling=simulator.proposal_sampling,
                simulator=simulator,
                scorer=scorer,
            )
            all_file_scores[token] = pdm_result.score
        ...
        with open(file_name, "a") as f:
            f.write(json.dumps(all_file_scores) + "\n")
        return all_file_scores
