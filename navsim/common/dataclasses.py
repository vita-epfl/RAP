...
class Camera:
    ...
    rendered_image: Optional[npt.NDArray[np.float32]] = None
...
class Cameras:
    ...
    def from_camera_dict(
        ...
        return Cameras(
            cam_f0=data_dict.get("cam_f0",Camera()),
            cam_l0=data_dict.get("cam_l0",Camera()),
            cam_l1=data_dict.get("cam_l1",Camera()),
            cam_l2=data_dict.get("cam_l2",Camera()),
            cam_r0=data_dict.get("cam_r0",Camera()),
            cam_r1=data_dict.get("cam_r1",Camera()),
            cam_r2=data_dict.get("cam_r2",Camera()),
            cam_b0=data_dict.get("cam_b0",Camera()),
        )
...
class Scene:
    ...
    def get_history_trajectory(self, num_trajectory_frames: Optional[int] = None) -> Trajectory:
        ...
        for frame_idx in range(num_trajectory_frames):
            try:
                ...
                global_ego_poses.append(self.frames[num_trajectory_frames-1].ego_status.ego_pose)
    ...
    def get_agent_input(self) -> AgentInput:
        ...
        for frame_idx in range(self.scene_metadata.num_history_frames):
            try:
                ...
                frame_idx = self.scene_metadata.num_history_frames - 1
                frame_ego_status = self.frames[frame_idx].ego_status
                ego_statuses.append(
                    EgoStatus(
                        ego_pose=local_ego_poses[frame_idx],
    ...
    def from_scene_dict_list(
        ...
        for frame_idx in range(len(scene_dict_list)):
            if frame_idx < num_history_frames-1:
                frame=None
            else:
