...
class RAPAgent(AbstractAgent):
    ...
    def rap_loss(self,targets: Dict[str, torch.Tensor], pred: Dict[str, torch.Tensor], config  ):
    ...
    def compute_loss(
        ...
        return self.rap_loss(targets, pred, self._config)

    def get_optimizers(self):
        vit_lr = self._lr * 0.2
        vit_params   = list(self._rap_model._backbone.img_backbone.parameters())
        other_params = [p for n, p in self._rap_model.named_parameters()
